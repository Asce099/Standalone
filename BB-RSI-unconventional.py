...
# ,'15minute', "minute", "5minute", "38minute" 
select_tf = ["day", "60minute","38minute"]
tables = [] 
final_data = {}
results_directory = 'Results/BBRSI-UC'
    ...
    def process_data(self, df):
        ...
        for i in range(len(self.df)):
            if i < 200:  # Adjust the skip condition to accommodate SMA_50
                ...
                continue
            elif (self.df.loc[i, 'Lower Bollinger Band'] > self.df.loc[i, 'close'] ) and (any(self.df.loc[i-j, 'RSI'] < 40 for j in range(0, 11))) and not self.entry_flag:
