...
tables = [] 
final_data = {}
results_directory = 'Results/MARSI-UC2'
    ...
    def process_data(self, df):
        ...
        for i in range(len(self.df)):
            if i < 200:  # Adjust the skip condition to accommodate SMA_50
                ...
                continue
            elif ((self.df.loc[i, 'SMA_20'] > self.df.loc[i, 'EMA_50'] and self.df.loc[i-1, 'SMA_20'] < self.df.loc[i, 'EMA_50']) or (self.df.loc[i, 'SMA_50'] > self.df.loc[i, 'EMA_200'] and self.df.loc[i-1, 'SMA_50'] < self.df.loc[i, 'EMA_200'])) and (any(self.df.loc[i-j, 'RSI'] < 40 for j in range(0, 11)))  and not self.entry_flag:
